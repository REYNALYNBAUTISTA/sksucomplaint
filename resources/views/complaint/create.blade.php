    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
                ...
                <form method="POST" action="{{ route('complaint.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select id="target_office_id" name="target_office_id" required
                            ...
                            @foreach ($offices as $office)
                                <option value="{{ $office->id }}" {{ old('target_office_id') == $office->id ? 'selected' : '' }}>
                                    {{ $office->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="6" required
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-3 focus:ring-green-500 focus:border-green-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label for="file_upload" class="block text-sm font-medium text-gray-700">Supporting Document / Evidence (Optional)</label>
